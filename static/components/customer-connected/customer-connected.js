(function (angular) {
  ...
  angular.module('manage')
    .component('customerConnected', {
      templateUrl: '/static/components/customer-connected/customer-connected.html',
  ...
  function customerConnectedCtrl($scope, erp, $timeout) {
    ...
    $scope.$on('customer-list', function (ev, data) {
      if (data.flag == 'show-customer-connected') {
        ...
        for (var i = 0; i < res.length; i++) {
          ...
          if (res[i].logiCoun.length > 0) {
            for (var j = 0; j < res[i].logiCoun.length; j++) {
              ...
              var shipPriceDis = erp.cacuDiscount(res[i].logiCoun[j].logisticsPrice, res[i].logiDiscount);
    ...
    $scope.submitWuliu = function () {
      ...
      var defaultWuliu = {};
      if ($scope.isconlist) {
        sendJson.accpid = $scope.setWuliuItem.ACCPID;
        sendJson.shopId = $scope.setWuliuItem.shopid;
        sendJson.accid = $scope.nowOpeUserId;
        sendUrl = 'app/rebate/newModifyAccProduct';
      }
    ...
    $scope.goActaddCoun = function () {
      ...
      erp.postFun2('lc/erpBusiness/getFormula', JSON.stringify({
        ...
        var shipPriceDisTem = erp.cacuDiscount(shipPriceTem, operateArr.wuliuCountTem);
    ...
    $scope.freshShipCost = function (item1, index1) {
      ...
      var nowPrice;
      for (var i = 0; i < $scope.wuliuArr[index1].countryArr.length; i++) {
        nowPrice = erp.cacuDiscount($scope.wuliuArr[index1].countryArr[i].shipPriceTem, $scope.wuliuArr[index1].wuliuCountTem);
    ...
    $scope.getShipCostGs = function (item2, index2, index1) {
      ...
      erp.postFun2('lc/erpBusiness/getFormula', JSON.stringify({
        ...
        var disPrice = erp.cacuDiscount(nowPrice, $scope.wuliuArr[index1].wuliuCountTem);
    ...
    $scope.changeSellRate = function () {
      $scope.editProItem.discountPrice = erp.cacuDiscount($scope.editProItem.SELLPRICE, $scope.sendData.discount || 0);
    ...
    $scope.guanlianshangpin = function (item) {
      ...
      $scope.guanlian = true;
      $scope.isconlist = true;
      extcommon(item);
      erp.postFun('app/shop/getshop', { "data": JSON.stringify({ userId: item.id || item.ID }) }, function (data) {
        console.log(data);
        if (data.data.statusCode == 200) {
          $scope.userStoreList = JSON.parse(data.data.result).shoplist;
          console.log($scope.userStoreList);
        } else {
          layer.msg('获取店铺列表错误');
        }
      });
    ...
    $scope.getguanlianva = function (item, index) {
      ...
      postUrl = "app/rebate/getaccproductdetaill";
      postData = { "data": "{'pid': '" + item.PID + "','accpid': '" + item.ACCPID + "','userId': '" + $scope.nowOpeUserId + "'}" };
      console.log(index)
      erp.postFun(postUrl, postData, function (n) {
        ...
        angular.forEach(obj.list, function (data) {
          data.flag = false;
          data.discountPrice = erp.cacuDiscount(data.SELLPRICE, data.SELLDISCOUNT);
          // data.shipCostDiscount = erp.cacuDiscount(data.erpdeltPrice, data.FREIGHTDISCOUNT);
    ...
    $scope.baocunedit = function (item, index, e) {
      ...
      erp.postFun("app/rebate/modifyaccproduct", { "data": "{'accpid': '" + item.ACCPID + "','sku':'" + item.SKU + "','selldiscount':'" + sellDis + "','accid': '" + $scope.nowOpeUserId + "','shopId': '" + item.shopid + "'}" }, son, err);
      return;
      ...
      function son(n) {
        ...
        $scope.excellist[index].discountPrice = erp.cacuDiscount($scope.excellist[index].SELLPRICE, sellDis);
    ...
    $scope.baocunedit2 = function (item, index2, e, index1) {
      var sellDis = $scope.sendData.discount || 0;
      erp.postFun("app/rebate/modifyaccproductdetail", { "data": "{'id': '" + item.ID + "','selldiscount':'" + sellDis + "','accid': '" + $scope.nowOpeUserId + "'}" }, son, err);
      return;
      ...
      function son(n) {
        ...
        $scope.excellist[index1].vlist[index2].discountPrice = erp.cacuDiscount(item.SELLPRICE, sellDis);
    ...
    function settleTanList(obj) {
      ...
      if ($scope.isconlist) {
        for (var i = 0; i < obj.list.length; i++) {
          ...
          obj.list[i].discountPrice = erp.cacuDiscount(obj.list[i].SELLPRICE, obj.list[i].SELLDISCOUNT);
          ...
          if (obj.list[i].erpdeltPrice) {
            obj.list[i].shipCostDiscount = erp.cacuDiscount(obj.list[i].erpdeltPrice, obj.list[i].FREIGHTDISCOUNT);
    ...
    function settleSendDataTan() {
      ...
      if ($scope.isconlist) {
        postUrl = "app/rebate/getaccproduct";
        sendData.data.search = $scope.searchinfoTan;
      }
