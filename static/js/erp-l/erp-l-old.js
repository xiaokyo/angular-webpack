(function () {
    ...
    app.controller('dpphdCtrl', ['$scope', 'erp', '$http', '$routeParams', function ($scope, erp, $http, $routeParams) {
        ...
        function getList() {
            ...
            var csData = {};
            var urlVal;
            csData.page = $scope.pagenum + '';
            csData.pageNum = $scope.pagesize;
            csData.account = $scope.account;
            csData.trackcode = $scope.trackCode;
            csData.days = $scope.days;
            csData.type = $scope.type;
            csData.packageUser = $scope.packageUser;
            csData.weight = $scope.weight;
            csData.beginDate = $('#left-time').val();
            csData.endDate = $('#right-time').val();
            if ($scope.dppStu == 0 || $scope.dppStu == 1) {
                csData.freightStatus = $scope.dppStu == 0 ? '201' : '200';
                csData.store = $scope.cliStore;
                urlVal = 'app/erplogistics/getFreightBySystemNotDataList';
            } else if ($scope.dppStu == 2 || $scope.dppStu == 3) {
                csData.freightStatus = $scope.dppStu == 2 ? '201' : '200';
                urlVal = 'app/erplogistics/getFreightNotDataList';
            } else if ($scope.dppStu == 4) {
                csData.freightStatus = '300';
                urlVal = 'app/erplogistics/getFreightBySystemNotDataList';
            }
        ...
        $scope.setLocalFun = function (val) {
            if (val) {
                ...
                $scope.inpVal = '';
                erp.postFun('erp/faHuo/addOutRecord', JSON.stringify(codeData), function (data) {
                    layer.closeAll('loading');
                    if (data.data.result.isExceptin) {
                        layer.msg('重量异常')
                    } else if (data.data.result.result > 0) {
                        layer.msg('提交成功')
                        // $('.audio-succ').get(0).play();
                        var huoDaiName = data.data.result.huodai;
                        ...
                        for (var i = 0; i < len; i++) {
                            ...
                            if (data.data.result.trackingNumCode == $scope.list[i].trackNum) {
                                $scope.list[i].hdName = huoDaiName;
                        ...
                        if (data.data.result.orderRepeat && data.data.result.orderRepeat > 0) {
                        ...
                        for (var i = 0; i < len; i++) {
                            if (data.data.result.trackingNumCode == $scope.list[i].trackNum) {
                                $scope.list[i].flag = true;
                                $scope.list[i].date = data.data.result.date;
                            }
                        }
                    }
                    else if (data.data.result.result == '过期面单') {
                        ...
                        for (var i = 0; i < len; i++) {
                            if (data.data.result.trackingNumCode == $scope.list[i].trackNum) {
                                $scope.list[i].flag = true;
                                $scope.list[i].desc = data.data.result.message;
                            }
                        }
                    }
                    else if (data.data.result.result == '这个包裹的货代错误.请查证后重新录入') {
                    ...
                    // else if(data.data.result.result=='该追踪号无法查找到订单,可能已经换单,请处理'){
                    ...
                    //       if(data.data.result.trackingNumCode==$scope.list[i].trackNum){
                    //           $scope.list[i].flag = true;
                    //           $scope.list[i].desc = data.data.result.result;
                    //       }
                    //      }
                    // }
                    else if (data.data.result.result == '该追踪号无法查找到订单,已经换单,请处理') {
                    ...
                    else if (data.data.result.result == '拦截') {
                        if ($scope.selStore == '2') {
                    ...
                    else if (data.data.result.result == '法国不能到地区拦截') {
                    ...
                    else if (data.data.result.result == '重复') {
                        $('.audio-en-cf').get(0).play();
                        var len = $scope.list.length;
                        for (var i = 0; i < len; i++) {
                            if (data.data.result.trackingNumCode == $scope.list[i].trackNum) {
                    ...
                    else if (data.data.result.result == '未发货') {
        ...
        $scope.upLoadImg4 = function (files) {
            if ($scope.uphdSelhdName) {
                ...
                erp.upLoadImgPost('app/erplogistics/uploadFreightDataByExcel?account=' + accountName, formData, orsFun, dreFun)
        ...
        function queryData(url) {
            ...
            erp.postFun(url, data, function (res) {
                ...
                if (ret.statusCode == '200') {
                    ...
                    if(ret.result.outputList){
                        for(let i = 0,len = ret.result.outputList.length;i<len;i++){
                            if(ret.result.outputList[i].pathName){
                                ret.result.outputList[i].strogeName = ret.result.outputList[i].pathName.split('>')[0].split('：')[1]
                            }
                        }
                    }
                    $scope.table_data = ret.result;
                    console.log(ret.result.outputList)
                } else {
                    layer.msg(ret.msg);
                }
                setPagination();
    ...
    app.controller('PackagingStatisticsCtrl', ['$scope', 'erp', function ($scope, erp) {
        ...
        function getPacketList() {
            ...
            console.log(data);
            erp.load();
            erp.postFun('erp/order/packageTongji', JSON.stringify(data), function (data) {
                erp.closeLoad();
                console.log(data);
                if (data.data.statusCode == '200') {
                    var list = data.data.result;
