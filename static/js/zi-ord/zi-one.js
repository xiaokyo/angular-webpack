					...
					if (item.logisticName == 'ePacket') {//$(this).parent().parent().siblings('.wl-ord-td').children('.wl-name-p').text()
						...
						if(item.countryCode == 'GB'&&item.orderweight < 2000){
							...
							if(noEntryFlag && item.countryCode != 'CA'){
								...
								else if (item.countryCode == 'IT' || item.countryCode == 'SE' || item.countryCode == 'FR' || item.countryCode == 'BE' || item.countryCode == 'NL' || item.countryCode == 'AT') {
									$scope.sanTaiNum++;
									$scope.showindex = 33;
									if(noEntryFlag){
										yTGbDdIds = item.id + ',';
					...
					for (let i = 0, len = $scope.erporderList.length; i < len; i++) {
						if ($scope.erporderList[i].order.checkFlag) {
							...
							if (wlName == 'ePacket') {
								...
								else if(ordCountryCode == 'IT' || ordCountryCode == 'SE' || ordCountryCode == 'FR' || ordCountryCode == 'BE' || ordCountryCode == 'NL' || ordCountryCode == 'AT'){
									$scope.sanTaiNum++;
									if(isNoEntryFlag){
										yTGbDdIds += itemOrdId + ',';
									} else if(commonSpFlag){
