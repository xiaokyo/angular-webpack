;~function () {
		...
		//自定义repeat完成指令
		app.directive('ckbtFinish',function($timeout){
			return {
				restrict: 'A',
				link: function(scope,elem,attr){
					...
					if (scope.$last === true) {
						$timeout(function () {
							//向父控制器传递事件消息
							scope.isDropOpt && scope.$emit('ckbtFinishCallback', scope.isDropOpt)
    ...
    app.controller('warestockCtrl', ['$scope', "erp", '$location', '$routeParams','$timeout', function ($scope, erp, $location, $routeParams,$timeout) {
		...
		$scope.isWw = ['admin', '陈真', '庹章龙', '金仙娟', '龚莹', '廖倩'].includes(loginName)
		$scope.isDropOpt = ''
        
        $scope.isPermitPrivOpe = ['admin', '张琪', '刘思梦', '金仙娟', '庹章龙'].includes(loginName) || ['纠纷', '管理'].includes(job)
        ...
        getRootNode(erp,{
            ...
            // 过滤没有权限的仓库
            const filterStorageList = function(arr,storageList){
                console.log('filter',arr,storageList)
                const list = []
                const ids = storageList.map(_=>_.dataId)
                arr.forEach(function(item,index){
                    if(ids.includes(item.pCodeKey)){
                        list.push(item)
                    }
                })
                // return list
                console.log('filtered',list)
                $scope.folder.childrens = list
            }
            filterStorageList(result.childrens,erp.getStorage())

            $scope.brunchItem = result;
            $scope.rootPaCodeId = $scope.brunchItem.pCodeId;
            let childrenStoreArr = result.childrens;
            let resIndex;//当前是默认选中哪个仓库
            if (loginSpace) {//如果有登陆地
                if (loginSpace == '深圳') {
                    resIndex = whereSpace('深圳仓',childrenStoreArr)
                } else if (loginSpace == '美国') {
                    resIndex = whereSpace('美东新泽西仓',childrenStoreArr)
                } else {
                    resIndex = whereSpace('义乌仓',childrenStoreArr)
                }
            } else {//否则走义乌
                resIndex = whereSpace('义乌仓',childrenStoreArr)
            }
            $scope.folder.childrens[resIndex].active = true;
            $scope.brunchItem = $scope.folder.childrens[resIndex];
        })
        function whereSpace(str,arr){
            console.log(arr.length)
            for(let i = 0,len = arr.length;i<len;i++){
                if(str==arr[i].codeName){
                    return i;
                }
            }
        }
        ...
        // 获取列表
        function getList(erp, $scope,pipei) {
            if($scope.brunchItem.codeName=='全部仓库'){
                layer.msg('不能用全部仓库查询,请选择具体仓库或者库位',{time:4000})
                return
            }
            if($scope.brunchItem.codeFlag!='0'&&!$scope.searchStr)return
            var sendJson = {};
            ...
            sendJson.storageDoId = $scope.brunchItem.pCodeKey,
            sendJson.skuOrCode = searchStr;
            if($scope.brunchItem.codeFlag!=0){
                sendJson.regionPathId = '';
            }else{
                sendJson.regionPathId = $scope.brunchItem.id;
            }
            erp.loadPercent($('.tab-con-wrap'), 750);
            $scope.productList = [];
            erp.postFun("storehouseSimple/goodsInfo/getOfferingInfoList", JSON.stringify(sendJson), function (data) {
                // erp.closeLoad();
                ...
                if ($scope.totalNum == 0) {
                    ...
                    return;
								...
								$scope.isDropOpt = ''
                if(pipei){//有第三参 匹配是否打开详情
                    for(let i = 0,len = $scope.lastProductArr.length;i<len;i++){
                        $scope.productList[i].detailFlag = $scope.lastProductArr[i].detailFlag;
                        if($scope.lastProductArr[i].detailFlag){
														$scope.productList[i].variantInventoryInfo = sjb($scope.productList[i].variantInventoryInfo)
														$scope.isDropOpt = $scope.productList[i].pid
                        }
                    }
                }
            }, err);
        ...
        $scope.showVariant = function (item, index1) {
            ...
            // }, err);
						...
						$scope.isDropOpt = item.detailFlag ? item.pid : ''
            console.log(item.variantInventoryInfo)
            if(item.variantInventoryInfo&&!item.variantInventoryInfo[0].arr){
                item.variantInventoryInfo = sjb(item.variantInventoryInfo)
            }
        }
        function sjb (arrData){
            ...
            res.forEach(function (a, b) {
                ...
                a.arr.forEach(function (k, j) {
                    var type = k.pathName.split('>')[0];
                    if (hash1[type]) {
                        ace[hash1[type] - 1].arr.push(k);
												var num = null;
												var occupiedInventoryNum = null
												var totalInventoryNum = null
                        ace[hash1[type] - 1].arr.forEach(function (o, i) {
														num += Number(o.goodsNum);
														occupiedInventoryNum += +o.occupiedInventory
														totalInventoryNum += +o.totalInventory
                        })
												ace[hash1[type] - 1].goodsNum = num
												ace[hash1[type] - 1].occupiedInventory = occupiedInventoryNum
												ace[hash1[type] - 1].totalInventory = totalInventoryNum
                    } else {
                        hash1[type] = ++j && ace.push({
                            arr: [k],
                            type: type,
														goodsNum: k.goodsNum,
														occupiedInventory: k.occupiedInventory,
														totalInventory: k.totalInventory
                        })
                    }
        ...
        $scope.turnToPriv = function (itemv, item) {
            ...
            $scope.ownerFlag = itemv.ownerFlag;
        }
        ...
        $scope.goActChanVarStatus = function () {
            ...
            console.log($scope.nowOpeVariant,$scope.turnPrivCust)
            var postUrl;
            var postData = {};
            ...
            postData.num = $scope.chanVarNum;
            postData.dbgoodId = $scope.nowOpeVariant.id;
            // postData.ownerId = $scope.turnPrivCust.id;
            postData.ownerFlag = $scope.ownerFlag;
            postUrl = 'storehouseSimple/goodsInfo/inventoryNumShiftOpt'
            // erp.load();
            layer.load(2);
            erp.postFun(postUrl, JSON.stringify(postData), function (data) {
                ...
                layer.msg('操作成功')
                $scope.lastProductArr = JSON.parse(JSON.stringify($scope.productList))
                getList(erp, $scope,1);
                console.log($scope.lastProductArr)
        ...
        $scope.goOpeStock = function () {
            ...
            if ($scope.inStockFlag) {
                ...
                postUrl = 'storehouseSimple/goodsInfo/inInventoryNumOpt';
            }
            if ($scope.outStockFlag) {
                postUrl = 'storehouseSimple/goodsInfo/outInventoryNumOpt'
            ...
            erp.postFun(postUrl, JSON.stringify({
                dbgoodId: $scope.nowOpeVariant.id,
                num: $scope.opeStockNum,
                remark: $scope.opeStockRemark,
                ownerFlag: $scope.ownerFlag
            }), function (data) {
                console.log(data);
                if (data.data.code != 200) {
                    if(data.data.code==401){
                ...
                // console.log(count);
                $scope.productList[$scope.nowOpePro.index].goodsNum = $scope.productList[$scope.nowOpePro.index].goodsNum * 1 + count;
                console.log($scope.productList[$scope.nowOpePro.index].variantInventoryInfo)
                $scope.productList[$scope.nowOpePro.index].variantInventoryInfo[$scope.nowOpeVariant.index].arr[$scope.idx3].goodsNum = $scope.productList[$scope.nowOpePro.index].variantInventoryInfo[$scope.nowOpeVariant.index].arr[$scope.idx3].goodsNum  * 1 + count;
                $scope.productList[$scope.nowOpePro.index].variantInventoryInfo[$scope.nowOpeVariant.index].arr[$scope.idx3].arr[$scope.idx4].goodsNum = $scope.productList[$scope.nowOpePro.index].variantInventoryInfo[$scope.nowOpeVariant.index].arr[$scope.idx3].arr[$scope.idx4].goodsNum * 1 + count;
                $scope.opeStockNum = '';
        ...
        $scope.getCode = function (e) {
            if (e.keyCode == 13) {
                if (e.target.id == 'sku-code') {
                    ...
                    let codeArr = $scope.code.split('-');
                    let code='';
                    codeArr.forEach((item,index)=>{
                        code+=item.trim()+(index<codeArr.length-1?'-':'');
                    })
                    $scope.code = code;
                    layer.load(2);
                    erp.postFun('storehouse/warehousOperation/getGoodsInfoByScanCode', JSON.stringify({
        ...
        $scope.goAddPosi = function () {
            ...
            layer.load(2);
            erp.postFun('warehouseBuildWeb/management/getRegionInfoByScanCode', JSON.stringify({
                ...
                $scope.posiList.unshift(data.data.regionInfoMap);
                angular.element('#posi-code').val('');
                $scope.posiCode = '';
                setTimeout(function () {
                    angular.element('.inp-stock-num').eq(0).focus();
        ...
        $scope.goOpeStockByCode = function () {
            ...
            for (var i = 0; i < $scope.posiList.length; i++) {
                if ($scope.posiList[i].optVal&&$scope.posiList[i].optVal>0) {
                    regionList.push({
                        regionPath: $scope.posiList[i].regionBarcode,
                        quantity: $scope.posiList[i].optVal
                    });
                }
            }
            ...
            $scope.reqDate.batchNumber = '';
            $scope.reqDate.regionList = regionList;
            erp.postFun('storehouse/warehousOperation/warehousing', $scope.reqDate, function (data) {
                layer.closeAll('loading');
                var data = data.data;
                console.log(data);
        ...
        $scope.query = () => {
            ...
            erp.postFun('storage/storageBasic/kuCunChaXun', JSON.stringify(erpData), function (data) {
				...
				/** 2020-4-18 库存库位展示优化 */
				$scope.$on('ckbtFinishCallback', (_, pid) => {
					const index = $scope.productList.findIndex(_ => _.pid === pid)
					// console.log(pid, index)
					//tab-con
					const wrapList = document.getElementsByClassName('tab-con') || []
					// console.log(wrapList[index])
					if(wrapList.length > 0) {
						const sjbList = wrapList[index].querySelectorAll('.sjb .cangwei .list')
						console.log(sjbList)
						sjbList.forEach((_, idx) => {
							console.log('clientHeight =>', _.clientHeight, ', offsetHeight =>', _.offsetHeight, ' , id =>', _.id)
							$scope.productList[index].variantInventoryInfo[idx].isHidden = _.clientHeight > 155 + 10
							$scope.productList[index].variantInventoryInfo[idx].isHiddenFlag = _.clientHeight > 155 + 10
						})
						console.log($scope.productList)
					}
					
				})
				$scope.loadMoreFn = item => {
					item.isHiddenFlag = !item.isHiddenFlag
				}
    }]);
    function err(error) {
        console.log(error);
